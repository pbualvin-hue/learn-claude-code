...
⏱️ **5 分鐘時限紀律（2026-07-15 實測：排程 session 自啟動起整 5 分鐘會被硬性終止，無警告）**：
draft PR 是本流程唯一不可失的交付物，所以順序是**先開 PR、再驗證**——內容一寫完就 commit＋push＋開 PR，驗證（build／連結／掃描）放在 PR 開出之後，結果用 `gh pr comment` 補報。就算驗證段被時限截斷，PR 已在、審核者照 PR 檢查清單補跑即可。草稿求精簡準確（60–90 行，審核時再潤），不要為了寫得漂亮把時間耗光。
...
```
## 審核檢查清單
- [ ] 隨機抽 3 條功能描述，對照官方文件無誤（黃金抽查——用下方「抽查錨點」的術語在兩邊 Ctrl+F 定位比對）
- [ ] （若觸發）決策比較頁複審結果已確認，見下節
- [ ] 資安相關頁面若被觸及：全數人工複核（必抽，不進隨機池）
- [ ] 本地驗證（build＋check-links＋掃描）——若下方沒有「驗證通過」留言，代表 run 被時限截斷，審核者需自行跑一次
- [ ] merge 後確認 footer 內容基準版本跳新

## 本次增量
<版本區間與條目數>

## 抽查錨點（黃金抽查用：每條解讀附一個「PR 與官方 changelog 兩邊都能 Ctrl+F 搜到」的英文術語）
<清單：術語｜對應解讀條目｜所在版本。內文的指令與功能名本就保留英文（站規），錨點直接取自各條解讀；哪一條找不出可搜的英文術語，代表該條無法回溯官方原文，必須標「⚠ 未驗證」或刪除>
...
全過 → `gh pr comment <PR編號> --body "本地驗證通過：build ✓ check-links ✓ 掃描 ✓"`。
有問題 → 修正、commit、push（同分支，PR 自動更新），再留言說明修了什麼。
